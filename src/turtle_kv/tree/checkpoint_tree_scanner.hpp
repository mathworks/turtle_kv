...
//=#=#==#==#===============+=+=+=+=++=++++++++++++++-++-+--+-+----+---------------
//
/** \brief Scanner for checkpoint trees.
 */
class CheckpointTreeScanner
{
 public:
  ...
  struct MergeTier {
    KeyView key;
    std::variant<NoneType, TreeTierState> state_impl;

    //----- --- -- -  -  -   -

    explicit MergeTier(const KVSlice& kv_slice, PathFrame* frame, i32 level_i) noexcept
        : key{get_key(kv_slice.front())}
        , state_impl{TreeTierState{
              .kv_slice = kv_slice,
              .frame = frame,
              .level_i = level_i,
          }}
    ...
    bool advance()
    {
      return batt::case_of(
          ...
          [this](TreeTierState& state) {
            state.kv_slice.drop_front();
            if (state.kv_slice.empty()) {
              state.kv_slice = state.frame->pull_next(state.level_i);
              if (state.kv_slice.empty()) {
                return false;
              }
            }
  ...
  struct MergeTierMinHeapOrder {
    bool operator()(MergeTier* left, MergeTier* right) const
    {
  ...
  struct PathFrame {
        ...
        , pivot_i_{(i32)in_node(node).find_pivot_containing(tree_scanner.min_key_)}
    {
      ...
      for (i32 level_i = 0; level_i < n_levels; ++level_i) {
        ...
        if (!first_slice.empty()) {
          ...
          MergeTier& tier = tree_scanner.tiers_.emplace_back(first_slice, this, level_i);
          if (kInsertHeap) {
        ...
        , pivot_i_{0}
    {
      ...
      MergeTier& tier = tree_scanner.tiers_.emplace_back(first_slice, this, 0);
      if (kInsertHeap) {
        tree_scanner.heap_.insert(&tier);
      }
    }
 ...
 private:
  ...
  Status set_next_item()
  {
    for (;;) {
      ...
      MergeTier* tier = this->heap_.first();

      if (!this->next_item_) {
        this->next_item_.emplace(tier->key, tier->value());

      } else if (this->next_item_->key == tier->key) {
        if (this->next_item_->needs_combine()) {
          this->next_item_->value = combine(this->next_item_->value, tier->value());
        }
  ...
  boost::container::static_vector<MergeTier, kMaxHeapSize> tiers_;
  StackMerger<MergeTier, MergeTierMinHeapOrder, kMaxHeapSize> heap_;
};
