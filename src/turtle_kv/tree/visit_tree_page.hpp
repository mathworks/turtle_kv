...
StatusOr<R> visit_tree_page(llfs::PageLoader& page_loader, llfs::PinnedPage& pinned_page_out,
                            const llfs::PageIdSlot& page_id_slot, VisitorFn&& visitor_fn) noexcept
{
  if (!pinned_page_out || pinned_page_out.page_id() != page_id_slot.page_id) {
    BATT_ASSIGN_OK_RESULT(
        pinned_page_out,
        page_id_slot.load_through(page_loader, /*required_layout=*/None,
...
template <
    typename VisitorFn,
    typename R = StatusOr<RemoveStatusOr<std::invoke_result_t<VisitorFn, const PackedLeafPage&>>>>
StatusOr<R> visit_tree_page(llfs::PageLoader& page_loader, const llfs::PageIdSlot& page_id_slot,
...
decltype(auto) visit_tree_page(llfs::PageLoader& page_loader, llfs::PinnedPage& pinned_page_out,
                               const llfs::PageIdSlot& page_id_slot, CaseFns&&... case_fns) noexcept
{
  return visit_tree_page(page_loader, pinned_page_out, page_id_slot,
...
//
template <typename... CaseFns>
decltype(auto) visit_tree_page(llfs::PageLoader& page_loader, const llfs::PageIdSlot& page_id_slot,
                               CaseFns&&... case_fns) noexcept
{
  return visit_tree_page(page_loader, page_id_slot,
                         batt::make_case_of_visitor(BATT_FORWARD(case_fns)...));
}
...
template <
    typename VisitorFn,
    typename R = StatusOr<RemoveStatusOr<std::invoke_result_t<VisitorFn, const PackedLeafPage&>>>>
StatusOr<R> visit_leaf_page(llfs::PageLoader& page_loader, llfs::PinnedPage& pinned_page_out,
                            const llfs::PageIdSlot& page_id_slot, VisitorFn&& visitor_fn) noexcept
{
  if (!pinned_page_out || pinned_page_out.page_id() != page_id_slot.page_id) {
    BATT_ASSIGN_OK_RESULT(
        pinned_page_out,
        page_id_slot.load_through(page_loader, LeafPageView::page_layout_id(),
                                  llfs::PinPageToJob::kDefault, llfs::OkIfNotFound{false}));
  }
  return BATT_FORWARD(visitor_fn)(PackedLeafPage::view_of(pinned_page_out));
}
...
template <
    typename VisitorFn,
    typename R = StatusOr<RemoveStatusOr<std::invoke_result_t<VisitorFn, const PackedLeafPage&>>>>
StatusOr<R> visit_leaf_page(llfs::PageLoader& page_loader, const llfs::PageIdSlot& page_id_slot,
                            VisitorFn&& visitor_fn) noexcept
{
  llfs::PinnedPage pinned_page;
...
StatusOr<R> visit_node_page(llfs::PageLoader& page_loader, llfs::PinnedPage& pinned_page_out,
                            const llfs::PageIdSlot& page_id_slot, VisitorFn&& visitor_fn) noexcept
{
  if (!pinned_page_out || pinned_page_out.page_id() != page_id_slot.page_id) {
    BATT_ASSIGN_OK_RESULT(
        pinned_page_out,
        page_id_slot.load_through(page_loader, NodePageView::page_layout_id(),
                                  llfs::PinPageToJob::kDefault, llfs::OkIfNotFound{false}));
  }
  return BATT_FORWARD(visitor_fn)(PackedNodePage::view_of(pinned_page_out));
}
...
template <
    typename VisitorFn,
    typename R = StatusOr<RemoveStatusOr<std::invoke_result_t<VisitorFn, const PackedNodePage&>>>>
StatusOr<R> visit_node_page(llfs::PageLoader& page_loader, const llfs::PageIdSlot& page_id_slot,
                            VisitorFn&& visitor_fn) noexcept
{
  llfs::PinnedPage pinned_page;
