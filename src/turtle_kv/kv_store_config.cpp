...
std::vector<ConfigParam> initialize_config_params()
{
  ...
  static ScaleConversion<u64> capacity_gb_get_set{initial_capacity_bytes_field,
                                                  initial_capacity_bytes_field,
                                                  kGiB};
                    ...
                    KVStoreRuntimeOptions* runtime_options [[maybe_unused]])
{
  ...
  BATT_REQUIRE_OK(iter->second.parse_and_set(value, config, runtime_options));
