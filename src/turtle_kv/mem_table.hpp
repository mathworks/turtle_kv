...
{
 public:
  using ReadLockT = PtrLock<absl::ReaderMutexLock>;
  using WriteLockT = PtrLock<absl::WriterMutexLock>;
  ...
  usize key_count() noexcept
  {
    ReadLockT lock{this->mutex_};
    return this->hashed_.size();
  }

  void reserve(usize n) noexcept
  {
    WriteLockT lock{this->mutex_};
    this->hashed_.reserve(n);
  }

  Status put(const KeyView& key, const ValueView& value) noexcept;
 ...
 private:
  ...
  Optional<ValueView> get_impl(const KeyView& key) noexcept;
  ...
  absl::flat_hash_set<MemTableEntry, DefaultStrHash, DefaultStrEq> hashed_;
...
{
  this->status = batt::to_status(this->mem_table.context_->append_slot(
