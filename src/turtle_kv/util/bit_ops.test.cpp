...
TEST(BitOpsTest, FirstNextCount)
{
  ...
  for (usize case_i = 0; case_i < 100000; ++case_i) {
    ...
    while (bit_i < 64) {
      if (prev_i) {
        ASSERT_GT(bit_i, *prev_i) << BATT_INSPECT(std::bitset<64>{bit_set}) << BATT_INSPECT(count)
                                  << BATT_INSPECT(case_i);
      }
      prev_i = bit_i;
      ++count;
...
TEST(BitOpsTest, LastPrevCount)
{
  ...
  for (usize case_i = 0; case_i < 100000; ++case_i) {
    ...
    while (bit_i >= 0) {
      if (prev_i) {
        ASSERT_LT(bit_i, *prev_i) << BATT_INSPECT(std::bitset<64>{bit_set}) << BATT_INSPECT(count)
                                  << BATT_INSPECT(case_i);
      }
      prev_i = bit_i;
      ++count;
