...
\begin{itemize}
    ...
    \begin{enumerate}
        \item In the \texttt{SegmentedLevel} from the right node, shift each segment's \texttt{active\_pivot} bit set to the left by the number of pivots in the left node.
        \item Concatenate the segment vectors from both \texttt{SegmentedLevel}s.
        \item Iterate over this newly concatenated segment vector to identify duplicate segments that exist due to a previous split. For these duplicates, merge the \texttt{PiecewiseFilter}s ($\S$\ref{section:datastructureupdates:piecewisefilter}), take the union of the \texttt{active\_pivot} bit sets, and then erase the duplicate.
    \end{enumerate}
